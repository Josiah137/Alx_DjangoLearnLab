...
#settting app all instances and their    serilizer
...
class GenreViewSet(ModelViewSet):
    ...
    permission_classes = [IsAuthenticatedOrReadOnly] # we made it like users can add teh gener of a book only if they are authic/knowen user(loged in). so in other words if not loged in we can read(GET) but can not modify(PUT,POST..)